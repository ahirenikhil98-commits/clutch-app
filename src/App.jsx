...
const LoadingScreen = ({ t }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: 16 }}>
    ...
    <span style={{ fontFamily: "var(--karla)", fontSize: 14, color: t.textSec }}>Loading cars…</span>
  </div>
);

const ErrorScreen = ({ message, onRetry, t }) => (
  <div style={{ padding: "60px 24px", textAlign: "center" }}>
    <div style={{ fontSize: 36, marginBottom: 12 }}>⚠️</div>
    <div style={{ fontFamily: "var(--outfit)", fontSize: 18, fontWeight: 700, color: t.text, marginBottom: 8 }}>
      Could not load cars
    </div>
    <div style={{ fontFamily: "var(--karla)", fontSize: 13, color: t.textSec, marginBottom: 24 }}>{message}</div>
    <button onClick={onRetry} style={{
      padding: "12px 28px", borderRadius: 12, border: "none",
      background: t.accent, color: "#fff", cursor: "pointer",
      fontFamily: "var(--outfit)", fontSize: 14, fontWeight: 700,
    }}>Retry</button>
  </div>
);
    ...
    <div onClick={onClose} style={{
      ...
      <div className="su" onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <span style={{ fontFamily: "var(--outfit)", fontSize: 16, fontWeight: 800, color: t.text }}>Select Variant</span>
          <span style={{ fontFamily: "var(--karla)", fontSize: 12, color: t.textSec }}>{car.name}</span>
        </div>
            ...
            return (
              <button key={i} onClick={() => { onSelect(i); onClose(); }} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontFamily: "var(--outfit)", fontSize: 14, fontWeight: isActive ? 700 : 500, color: isActive ? t.accent : t.text }}>{vr.n}</span>
                </div>
                <span style={{ fontFamily: "var(--mono)", fontSize: 13, fontWeight: 600, color: isActive ? t.accent : t.textSec }}>{fmt(vr.p)}</span>
              </button>
            );
          })}
        </div>
...
const VariantTrigger = ({ car, vi, onOpen, t }) => (
  <button onClick={onOpen} style={{
    ...
    background: t.accentBg, border: `1px solid ${t.accent}30`, transition: "all 0.2s",
  }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{ fontSize: 9, fontFamily: "var(--mono)", color: t.textMuted, letterSpacing: "0.06em" }}>VARIANT</span>
      <span style={{ fontFamily: "var(--outfit)", fontSize: 13, fontWeight: 700, color: t.accent }}>{car.variants[vi].n}</span>
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{ fontFamily: "var(--mono)", fontSize: 12, fontWeight: 600, color: t.text }}>{fmt(car.variants[vi].p)}</span>
...
const CarCard = ({ car, budget, dp, salary, t, onSelect, onCompare, isCompared }) => {
  ...
  return (
    <div className="fu" style={{
      ...
      <div style={{ background: t.carBg, padding: "16px 20px 12px", borderBottom: `1px solid ${t.border}` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              <div style={{ fontSize: 9, fontFamily: "var(--mono)", fontWeight: 600, color: car.hue, background: car.hue + "15", padding: "2px 8px", borderRadius: 4, letterSpacing: "0.06em" }}>{car.brand.toUpperCase()}</div>
              <span style={{ fontSize: 9, fontFamily: "var(--mono)", color: zc, background: zbg, padding: "2px 8px", borderRadius: 4, fontWeight: 600 }}>{z.l.toUpperCase()}</span>
            </div>
            <div style={{ fontFamily: "var(--outfit)", fontSize: 17, fontWeight: 800, color: t.text, marginBottom: 4 }}>{car.name}</div>
      ...
      <VariantSheet open={sheetOpen} car={car} current={vi} onSelect={setVi} onClose={() => setSheetOpen(false)} t={t} />
...
/* ─── RESULTS SCREEN ────────────────────────────────────────── */
const ResultsScreen = ({ cars, salary, dp, exEMI, budget, t, onBack, onSelect, compared, onCompare }) => {
  const [seg, setSeg] = useState("All");
  const segs = ["All", "Hatchback", "Sedan", "Compact SUV", "SUV"];

  const filtered = cars.filter(c => {
    const base = c.variants[0].p;
    return base <= budget.maxBudget && (seg === "All" || c.segment === seg);
      ...
      <div style={{ padding: "0 20px", position: "sticky", top: 0, zIndex: 10,
        ...
        <div className="fu fu1" style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            ...
            <div style={{ background: t.greenBg, border: `1px solid ${t.green}30`, borderRadius: 10, padding: "8px 12px", textAlign: "center" }}>
              <div style={{ fontFamily: "var(--outfit)", fontSize: 22, fontWeight: 900, color: t.green }}>{filtered.length}</div>
    ...
    <div style={{ padding: "0 20px 60px" }}>
      ...
      <div className="fu fu1" style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ width: 64, height: 64, borderRadius: 16, background: t.carBg, display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${t.border}` }}>
      ...
      <div className="fu fu2" style={{
        ...
        <div style={{ fontSize: 10, fontFamily: "var(--mono)", color: t.textMuted, letterSpacing: "0.08em", marginBottom: 8 }}>YOUR MONTHLY EMI</div>
        <div style={{ fontFamily: "var(--outfit)", fontSize: 52, fontWeight: 900, color: mc, lineHeight: 1 }}>{fmtFull(emi)}</div>
        ...
        <div style={{ marginTop: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, fontFamily: "var(--mono)", color: mc, fontWeight: 600 }}>{pct}% of salary · {ml}</span>
      ...
      <div className="fu fu3" style={{
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {tenures.map(tn => <Pill key={tn} active={tenure===tn} onClick={() => setTenure(tn)} t={t}>{tn}m</Pill>)}
          </div>
        </div>
        <div style={{ height: 1, background: t.border, margin: "0 0 20px" }} />
        <div>
      ...
      <div className="fu fu4" style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 18, padding: "16px 20px", marginBottom: 20 }}>
...
const CompareTray = ({ compared, onRemove, onOpen, t }) => {
  ...
  return (
    <div className="su" style={{ position: "fixed", bottom: 0, left: 0, right: 0, display: "flex", justifyContent: "center", zIndex: 100, pointerEvents: "none" }}>
      <div style={{
        width: "100%", maxWidth: 430, pointerEvents: "auto",
        background: t.surface, borderTop: `1.5px solid ${t.accent}40`,
        borderRadius: "16px 16px 0 0", padding: "14px 16px 18px", boxShadow: `0 -4px 24px rgba(0,0,0,0.18)`,
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <span style={{ fontFamily: "var(--outfit)", fontSize: 13, fontWeight: 700, color: t.text }}>Compare ({compared.length}/3)</span>
          {compared.length >= 2 && (
            <button onClick={onOpen} style={{ padding: "8px 18px", borderRadius: 10, border: "none", background: t.accent, color: "#fff", cursor: "pointer", fontFamily: "var(--outfit)", fontSize: 12, fontWeight: 700 }}>Compare Now →</button>
          )}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {compared.map((c, i) => (
            <div key={i} style={{ flex: 1, background: t.card, border: `1px solid ${t.border}`, borderRadius: 12, padding: "8px 10px", position: "relative" }}>
              <button onClick={() => onRemove(i)} style={{ position: "absolute", top: -6, right: -6, width: 20, height: 20, borderRadius: 99, background: t.red, color: "#fff", border: "none", fontSize: 10, cursor: "pointer", lineHeight: "20px" }}>✕</button>
              <div style={{ fontFamily: "var(--outfit)", fontSize: 11, fontWeight: 700, color: t.text, marginBottom: 2 }}>{c.name.split(" ").slice(1).join(" ")}</div>
              <div style={{ fontSize: 10, fontFamily: "var(--mono)", color: t.accent }}>{c.variants[c.vi].n} · {fmt(c.variants[c.vi].p)}</div>
            </div>
          ))}
          {Array.from({ length: 3 - compared.length }).map((_, i) => (
            <div key={`e${i}`} style={{ flex: 1, border: `1.5px dashed ${t.border}`, borderRadius: 12, padding: "14px 10px", textAlign: "center" }}>
...
const CompareModal = ({ compared, salary, dp, t, onClose }) => {
  const rows = [
    ...
    { l: "EMI % of Salary", fn: c => {
      ...
      return Math.round((e / salary) * 100) + "%";
    }},
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div className="su" onClick={e => e.stopPropagation()} style={{ background: t.bg, borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 430, maxHeight: "85vh", overflowY: "auto", padding: "24px 20px 40px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <span style={{ fontFamily: "var(--outfit)", fontSize: 20, fontWeight: 900, color: t.text }}>Compare Cars</span>
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 99, background: t.card, border: `1px solid ${t.border}`, cursor: "pointer", fontSize: 14, color: t.textSec }}>✕</button>
        </div>

        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {compared.map((c, i) => (
            <div key={i} style={{ flex: 1, textAlign: "center" }}>
              <div style={{ background: t.carBg, borderRadius: 14, padding: "12px 8px", marginBottom: 8, border: `1px solid ${t.border}` }}>
        ...
        {rows.map((row, ri) => (
          <div key={ri} style={{ display: "flex", borderBottom: `1px solid ${t.border}` }}>
            <div style={{ width: 90, flexShrink: 0, padding: "10px 0", fontSize: 11, fontFamily: "var(--karla)", fontWeight: 600, color: t.textSec }}>{row.l}</div>
            {compared.map((c, ci) => (
              <div key={ci} style={{ flex: 1, padding: "10px 6px", textAlign: "center", fontFamily: "var(--mono)", fontSize: 12, fontWeight: 500, color: t.text }}>{row.fn(c)}</div>
            ))}
          </div>
        ))}
      </div>
    ...
    <>
      ...
      <div style={{
        ...
        {screen === "results" && (
          ...
          loadErr  ? <ErrorScreen message={loadErr} onRetry={fetchCars} t={t} /> :
          <ResultsScreen cars={cars} salary={salary} dp={dp} exEMI={exEMI} budget={budget} t={t}
        ...
        {screen === "results" && !loading && !loadErr && (
